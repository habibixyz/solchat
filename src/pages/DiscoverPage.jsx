...
export default function Discover() {
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: C.textDim, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{name}</div>
                </div>
              </div>
  ...
  const TokenDetail = () => {
    ...
    function TradesPanel({ buys, sells, buyPct }) {
      return (
  ...
  return (
    <div style={{ position: "fixed", top: 64, left: 0, right: 0, bottom: 0, display: "flex", flexDirection: "column", fontFamily: mono, background: C.bg, overflow: "hidden" }}>
      ...
      <div style={{ height: 40, borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", padding: "0 14px", gap: 12, flexShrink: 0, background: "rgba(0,247,255,0.015)" }}>
        <div style={{ fontSize: 12, letterSpacing: 3, color: C.cyan, opacity: 0.5, textTransform: "uppercase", fontWeight: 700 }}>
        ...
        {!isMobile && (
          <div style={{ marginLeft: "auto", fontSize: 11, color: C.textDim, letterSpacing: 1 }}>
<span>{tokens.length} pairs · <span style={{ color: C.cyanDim }}>SOLANA</span></span>
          </div>
        )}
      </div>
      ...
      {isMobile ? (
        <div style={{ flex: 1, overflow: "hidden" }}>
          {mobileView === "list" ? <TokenList /> : selected && pair ? <TokenDetail /> : <TokenList />}
        </div>
      ) : (
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
