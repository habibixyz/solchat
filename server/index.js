...
// ============================================================================
// CLAIM USERNAME (SECURE BACKEND BYPASSING RLS)
// ============================================================================

app.post('/api/auth/claim-username', async (req, res) => {
  try {
    ...
    if (!isValid) {
      return res.status(401).json({ error: 'Invalid signature. Username not claimed.' });
    }
    ...
    if (fetchError) {
      console.error('Fetch username error:', fetchError);
    ...
    if (upsertError) {
      console.error('Username upsert error:', upsertError);
